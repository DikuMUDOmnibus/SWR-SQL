 ...
 */
void save_clan( CLAN_DATA * clan, int flags )
{
   char updatequery[MAX_STRING_LENGTH*2];
   char selectquery[MAX_STRING_LENGTH*2];
MYSQL_RES   *qresult;
sprintf(selectquery, "SELECT * FROM clans where id='%d'", clan->id);
qresult = query_db(selectquery);
if (!sql_data_returned(qresult))
   sprintf(updatequery, "INSERT INTO clans SET");
else
   sprintf(updatequery, "UPDATE clans SET");

   if (IS_SET(flags, CLANDATA_NAME) || IS_SET(flags, CLANDATA_ALL))
   {
        sprintf(updatequery, "%s name='%s',", updatequery, clan->name);
   }
   if (IS_SET(flags, CLANDATA_DESCRIPTION) || IS_SET(flags, CLANDATA_ALL))
   {
        sprintf(updatequery, "%s description='%s',", updatequery, clan->description);
   }
   if (IS_SET(flags, CLANDATA_LEADER) || IS_SET(flags, CLANDATA_ALL))
   {
        sprintf(updatequery, "%s leader='%s',", updatequery, clan->leader);
   }
   if (IS_SET(flags, CLANDATA_NUMBER1) || IS_SET(flags, CLANDATA_ALL))
   {
        sprintf(updatequery, "%s number1='%s',", updatequery, clan->number1);
   }
   if (IS_SET(flags, CLANDATA_NUMBER2) || IS_SET(flags, CLANDATA_ALL))
   {
        sprintf(updatequery, "%s number2='%s',", updatequery, clan->number2);
   }
   if (IS_SET(flags, CLANDATA_TYPE) || IS_SET(flags, CLANDATA_ALL))
   {
        sprintf(updatequery, "%s type='%d',", updatequery, clan->clan_type);
   }
   if (IS_SET(flags, CLANDATA_BOARD) || IS_SET(flags, CLANDATA_ALL))
   {
        sprintf(updatequery, "%s board='%d',", updatequery, clan->board);
   }
   if (IS_SET(flags, CLANDATA_STOREROOM) || IS_SET(flags, CLANDATA_ALL))
   {
        sprintf(updatequery, "%s storeroom='%d',", updatequery, clan->storeroom);
   }
   if (IS_SET(flags, CLANDATA_GUARD1) || IS_SET(flags, CLANDATA_ALL))
   {
        sprintf(updatequery, "%s guard1='%d',", updatequery, clan->guard1);
   }
   if (IS_SET(flags, CLANDATA_GUARD2) || IS_SET(flags, CLANDATA_ALL))
   {
        sprintf(updatequery, "%s guard2='%d',", updatequery, clan->guard2);
   }
   if (IS_SET(flags, CLANDATA_PATROL1) || IS_SET(flags, CLANDATA_ALL))
   {
        sprintf(updatequery, "%s patrol1='%d',", updatequery, clan->patrol1);
   }
   if (IS_SET(flags, CLANDATA_PATROL2) || IS_SET(flags, CLANDATA_ALL))
   {
        sprintf(updatequery, "%s patrol2='%d',", updatequery, clan->patrol2);
   }
   if (IS_SET(flags, CLANDATA_TROOPER1) || IS_SET(flags, CLANDATA_ALL))
   {
        sprintf(updatequery, "%s trooper1='%d',", updatequery, clan->trooper1);
   }
   if (IS_SET(flags, CLANDATA_TROOPER2) || IS_SET(flags, CLANDATA_ALL))
   {
        sprintf(updatequery, "%s trooper2='%d',", updatequery, clan->trooper2);
   }
   if (IS_SET(flags, CLANDATA_FUNDS) || IS_SET(flags, CLANDATA_ALL))
   {
        sprintf(updatequery, "%s funds='%ld',", updatequery, clan->funds);
   }
   if (IS_SET(flags, CLANDATA_JAIL) || IS_SET(flags, CLANDATA_ALL))
   {
        sprintf(updatequery, "%s jail='%d',", updatequery, clan->jail);
   }
   if (IS_SET(flags, CLANDATA_MKILLS) || IS_SET(flags, CLANDATA_ALL))
   {
        sprintf(updatequery, "%s mkills='%d',", updatequery, clan->mkills);
   }
   if (IS_SET(flags, CLANDATA_MAINCLAN) || IS_SET(flags, CLANDATA_ALL))
   {
        sprintf(updatequery, "%s mainclan='%s',", updatequery, clan->mainclan->name);
   }
   if (IS_SET(flags, CLANDATA_MDEATHS) || IS_SET(flags, CLANDATA_ALL))
   {
        sprintf(updatequery, "%s mdeaths='%d',", updatequery, clan->mdeaths);
   }
   if (IS_SET(flags, CLANDATA_MEMBERS) || IS_SET(flags, CLANDATA_ALL))
   {
        sprintf(updatequery, "%s members='%d',", updatequery, clan->members);
   }
   if (IS_SET(flags, CLANDATA_PDEATHS) || IS_SET(flags, CLANDATA_ALL))
   {
        sprintf(updatequery, "%s pdeaths='%d',", updatequery, clan->pdeaths);
   }
   if (IS_SET(flags, CLANDATA_PKILLS) || IS_SET(flags, CLANDATA_ALL))
   {
        sprintf(updatequery, "%s pkills='%d',", updatequery, clan->pkills);
   }
   updatequery[strlen(updatequery) - 1] = '\0';
   if (sql_data_returned(qresult))
   {
   sprintf(updatequery, "%s where id='%d'", updatequery, clan->id);
}
...
*/
void save_planet( PLANET_DATA * planet, int flags )
{
   char updatequery[MAX_STRING_LENGTH*2];
   char selectquery[MAX_STRING_LENGTH*2];
   char areas[MAX_STRING_LENGTH];
   AREA_DATA * pArea;
MYSQL_RES   *qresult;
MYSQL_ROW pdata;
sprintf(selectquery, "SELECT * FROM planets where id='%d'", planet->id);
qresult = query_db(selectquery);
if (!sql_data_returned(qresult))
   sprintf(updatequery, "INSERT INTO planets SET");
else
   sprintf(updatequery, "UPDATE planets SET");

   if (IS_SET(flags, PLANETDATA_STARSYSTEM) || IS_SET(flags, PLANETDATA_ALL))
   {
        sprintf(updatequery, "%s starsystem='%s',", updatequery, planet->starsystem->name);
   }
   if (IS_SET(flags, PLANETDATA_AREAS) || IS_SET(flags, PLANETDATA_ALL))
   {
             for( pArea = planet->first_area; pArea; pArea = pArea->next_on_planet )
             sprintf(areas, "%s %s", areas, pArea->filename);
        sprintf(updatequery, "%s areas='%s',", updatequery, areas);
   }
   if (IS_SET(flags, PLANETDATA_NAME) || IS_SET(flags, PLANETDATA_ALL))
   {
        sprintf(updatequery, "%s name='%s',", updatequery, planet->name);
   }
   if (IS_SET(flags, PLANETDATA_BASEVALUE) || IS_SET(flags, PLANETDATA_ALL))
   {
        sprintf(updatequery, "%s base_value='%ld',", updatequery, planet->base_value);
   }
   if (IS_SET(flags, PLANETDATA_GOVERNEDBY) || IS_SET(flags, PLANETDATA_ALL))
   {
        sprintf(updatequery, "%s governed_by='%s',", updatequery, planet->governed_by->name);
   }
   if (IS_SET(flags, PLANETDATA_POPULATION) || IS_SET(flags, PLANETDATA_ALL))
   {
        sprintf(updatequery, "%s starsystem='%d',", updatequery, planet->population);
   }
   if (IS_SET(flags, PLANETDATA_FLAGS) || IS_SET(flags, PLANETDATA_ALL))
   {
        sprintf(updatequery, "%s flags='%d',", updatequery, planet->flags);
   }
   if (IS_SET(flags, PLANETDATA_POPSUPPORT) || IS_SET(flags, PLANETDATA_ALL))
   {
        sprintf(updatequery, "%s pop_support='%f',", updatequery, planet->pop_support);
   }
   if (IS_SET(flags, PLANETDATA_XPOS) || IS_SET(flags, PLANETDATA_ALL))
   {
        sprintf(updatequery, "%s x='%d',", updatequery, planet->xpos);
   }
   if (IS_SET(flags, PLANETDATA_YPOS) || IS_SET(flags, PLANETDATA_ALL))
   {
        sprintf(updatequery, "%s y='%d',", updatequery, planet->ypos);
   }
   if (IS_SET(flags, PLANETDATA_ZPOS) || IS_SET(flags, PLANETDATA_ALL))
   {
        sprintf(updatequery, "%s z='%d',", updatequery, planet->zpos);
   }
   if (IS_SET(flags, PLANETDATA_TYPE) || IS_SET(flags, PLANETDATA_ALL))
   {
        sprintf(updatequery, "%s type='%d',", updatequery, planet->type);
   }
   if (IS_SET(flags, PLANETDATA_GRAVITY) || IS_SET(flags, PLANETDATA_ALL))
   {
        sprintf(updatequery, "%s gravity='%d',", updatequery, planet->gravity);
   }
   updatequery[strlen(updatequery) - 1] = '\0';
   if (sql_data_returned(qresult))
    sprintf(updatequery, "%s where id='%d'", updatequery, planet->id);
else
{
    sprintf(selectquery, "SELECT * FROM planets order by id desc");
qresult = query_db(selectquery);
pdata = mysql_fetch_row(qresult);
planet->id = atoi(pdata[0]);
}
   query_db(updatequery);
   return;
}
void load_planets()
{
   ...
   PLANET_DATA *planet;
MYSQL_RES   *qresult;
MYSQL_ROW planetdata;//, vaultdata;
SPACE_DATA *starsystem;
   //   ROOM_INDEX_DATA *storeroom;
sprintf(selectquery, "SELECT * FROM planets");
qresult = query_db(selectquery);
...
{
while( (planetdata = mysql_fetch_row(qresult)) )
{
   CREATE( planet, PLANET_DATA, 1 );
planet->name = planetdata[3];
planet->id = atoi(planetdata[0]);
planet->base_value = atoi(planetdata[4]);
planet->population = atoi(planetdata[6]);
planet->flags = atoi(planetdata[7]);
planet->pop_support = atoi(planetdata[8]);
planet->xpos = atoi(planetdata[9]);
planet->ypos = atoi(planetdata[10]);
planet->zpos = atoi(planetdata[11]);
planet->type = atoi(planetdata[12]);
planet->gravity = atoi(planetdata[13]);
areas = planetdata[2];
//atok = strtok(areas, " ");
AREA_DATA *pArea;
atok = strtok(areas, " ");
               for( pArea = first_area; pArea; pArea = pArea->next )
               {
                  ...
                  if( pArea->filename && !str_cmp( pArea->filename, atok ) )
                  {
...
while ((atok = strtok(NULL, " ")) != NULL)
{
 ...
 planet->starsystem = starsystem_from_id( atoi(planetdata[1]) );
     ...
     //LINK( planet, planet->next_in_system, planet->prev_in_system, next, prev );
...
MYSQL_RES   *qresult;
//CHAR_DATA *victim;
MYSQL_ROW clandata;//, vaultdata;
   //   ROOM_INDEX_DATA *storeroom;
sprintf(selectquery, "SELECT * FROM clans");
qresult = query_db(selectquery);
...
{
while( (clandata = mysql_fetch_row(qresult)) )
{
   ...
   clan->mainclan = NULL;
clan->board = atoi(clandata[7]);
clan->description = clandata[2];
clan->funds = atoi(clandata[15]);
clan->guard1 = atoi(clandata[9]);
clan->guard2 = atoi(clandata[10]);
clan->jail = atoi(clandata[16]);
clan->leader = clandata[3];
clan->mdeaths = atoi(clandata[19]);
clan->mkills = atoi(clandata[17]);
clan->mainclan = get_clan(clandata[18]);
clan->members = atoi(clandata[20]);
clan->name = clandata[1];
clan->number1 = clandata[4];
clan->number2 = clandata[5];
clan->pdeaths = atoi(clandata[21]);
clan->pkills = atoi(clandata[22]);
clan->patrol1 = atoi(clandata[11]);
clan->patrol2 = atoi(clandata[12]);
clan->storeroom = atoi(clandata[8]);
clan->clan_type = atoi(clandata[6]);
clan->id = atoi(clandata[0]);
clan->trooper1 = atoi(clandata[13]);
clan->trooper2 = atoi(clandata[14]);
 /*  for( victim = first_char; victim; victim = victim->next )
      if( !str_cmp( victim->pcdata->clan->name, clan->name ) )
         clan->members++;*/
...
sprintf(selectquery, "SELECT * FROM clanvaults where clanid='%d'", clan->id);
qresult = query_db(selectquery);
vaultdata = mysql_fetch_row(qresult);
...
while( (vaultdata = mysql_fetch_row(qresult)) )
         {
fread_obj( supermob, fopen("clan.vault", "r"), OS_CARRY );

//load_obj( supermob, atoi(vaultdata[0]), OS_CARRY );
         }
...
void do_setclan( CHAR_DATA * ch, char *argument )
{
   ...
   if( !strcmp( arg2, "leader" ) )
   {
      ...
      send_to_char( "Done.\n\r", ch );
      save_clan( clan, CLANDATA_LEADER );
      return;
   }
   ...
   if( !strcmp( arg2, "subclan" ) )
   {
      ...
      int flags;
      SET_BIT(flags, CLANDATA_TYPE);
      SET_BIT(flags, CLANDATA_MAINCLAN);
      //save_clan( clan, flags );
      save_clan( subclan, flags );
      return;
   }
   ...
   if( !strcmp( arg2, "number1" ) )
   {
      ...
      send_to_char( "Done.\n\r", ch );
      save_clan( clan, CLANDATA_NUMBER1 );
      return;
   }
   ...
   if( !strcmp( arg2, "number2" ) )
   {
      ...
      send_to_char( "Done.\n\r", ch );
      save_clan( clan, CLANDATA_NUMBER2 );
      return;
   }
   ...
   if( !strcmp( arg2, "members" ) )
   {
      ...
      send_to_char( "Done.\n\r", ch );
      save_clan( clan, CLANDATA_MEMBERS );
      return;
   }
   ...
   if( !strcmp( arg2, "storage" ) )
   {
      ...
      send_to_char( "Done.\n\r", ch );
      save_clan( clan, CLANDATA_STOREROOM );
      return;
   }
   ...
   if( !strcmp( arg2, "guard1" ) )
   {
      ...
      send_to_char( "Done.\n\r", ch );
      save_clan( clan, CLANDATA_GUARD1 );
      return;
   }
   ...
   if( !strcmp( arg2, "guard2" ) )
   {
      ...
      send_to_char( "Done.\n\r", ch );
      save_clan( clan, CLANDATA_GUARD2 );
      return;
   }
   ...
   if( !strcmp( arg2, "trooper1" ) )
   {
      ...
      send_to_char( "Done.\n\r", ch );
      save_clan( clan, CLANDATA_TROOPER1 );
      return;
   }
   ...
   if( !strcmp( arg2, "trooper2" ) )
   {
      ...
      send_to_char( "Done.\n\r", ch );
      save_clan( clan, CLANDATA_TROOPER2);
      return;
   }
   if( !strcmp( arg2, "patrol1" ) )
   {
      ...
      send_to_char( "Done.\n\r", ch );
      save_clan( clan, CLANDATA_PATROL1 );
      return;
   }
   ...
   if( !strcmp( arg2, "patrol2" ) )
   {
      ...
      send_to_char( "Done.\n\r", ch );
      save_clan( clan, CLANDATA_PATROL2 );
      return;
   }
...
*/
   if( !strcmp( arg2, "desc" ) )
   {
      ...
      send_to_char( "Done.\n\r", ch );
      save_clan( clan, CLANDATA_DESCRIPTION );
      return;
   }
...
void do_setplanet( CHAR_DATA * ch, char *argument )
{
   ...
   if( arg1[0] == '\0' )
   {
      ...
      send_to_char( "\n\rField being one of:\n\r", ch );
      send_to_char( " base_value flags type\n\r", ch );
      send_to_char( " name starsystem governed_by\n\r", ch );
      return;
   }
   ...
   if( !strcmp( arg2, "name" ) )
   {
      ...
      send_to_char( "Done.\n\r", ch );
      save_planet( planet, PLANETDATA_NAME );
      return;
   }
   ...
   if( !strcmp( arg2, "governed_by" ) )
   {
      ...
      if( clan )
      {
         ...
         send_to_char( "Done.\n\r", ch );
         save_planet( planet, PLANETDATA_GOVERNEDBY );
      }
      else
         send_to_char( "No such clan.\n\r", ch );
      return;
   ...
   if( !strcmp( arg2, "starsystem" ) )
   {
      ...
      save_planet( planet, PLANETDATA_STARSYSTEM );
      return;
   }
/*
   if( !strcmp( arg2, "filename" ) )
...
*/
   ...
   if( !strcmp( arg2, "type" ) )
   {
      if( !str_cmp( argument, "star" ) )
         planet->type = PLANETTYPE_STAR;
      else if( !str_cmp( argument, "planet" ) )
         planet->type = PLANETTYPE_PLANET;
  //    else if( !str_cmp( argument, "black hole" ) )
   //      planet->type = PLANETTYPE_BLACKHOLE;
      else
         planet->type = PLANETTYPE_PLANET;
      send_to_char( "Done.\n\r", ch );
      save_planet( planet, PLANETDATA_TYPE );
      return;
   }
   if( !strcmp( arg2, "base_value" ) )
   {
      ...
      send_to_char( "Done.\n\r", ch );
      save_planet( planet, PLANETDATA_BASEVALUE );
      return;
   }
   ...
   if( !strcmp( arg2, "flags" ) )
   {
      ...
      for( ; farg[0] != '\0'; argument = one_argument( argument, farg ) )
      {
         if( !str_cmp( farg, "nocapture" ) )
            SET_BIT( planet->flags, PLANET_NOCAPTURE );
         else
            ch_printf( ch, "No such flag: %s\n\r", farg );
      }
      send_to_char( "Done.\n\r", ch );
     save_planet( planet, PLANETDATA_FLAGS );
      return;
   }
...
void do_showclan( CHAR_DATA * ch, char *argument )
{
   ...
   ch_printf( ch, "%s      : %s\n\r",
              ...
              clan->clan_type == CLAN_GUILD ? "Guild " : "Organization ", clan->name );
...
void do_appoint( CHAR_DATA * ch, char *argument )
{
   ...
   else if( !str_cmp( argument, "second" ) )
   {
      ...
      do_appoint( ch, "" );
   int flags;
   SET_BIT(flags, CLANDATA_NUMBER1);
   SET_BIT(flags, CLANDATA_NUMBER2);
   save_clan( ch->pcdata->clan, flags );
...
int flags;
SET_BIT(flags, PLANETDATA_GOVERNEDBY);
SET_BIT(flags, PLANETDATA_POPSUPPORT);
   save_planet( planet, flags );
