...
MYSQL_RES   *qresult;
MYSQL_ROW areadata;
sprintf(selectquery, "SELECT * FROM areas");
...
while( (areadata = mysql_fetch_row(qresult)) )
   {
   CREATE( pArea, AREA_DATA, 1 );
pArea->id = atoi(areadata[0]);
pArea->planet = planet_from_id(atoi(areadata[1]));
pArea->first_room = get_room_index(atoi(areadata[2]));
pArea->last_room = get_room_index(atoi(areadata[3]));
pArea->name = areadata[4];
pArea->flags = atoi(areadata[5]);
pArea->status = atoi(areadata[6]);
pArea->age = atoi(areadata[7]);
pArea->nplayer = atoi(areadata[8]);
pArea->reset_frequency = atoi(areadata[9]);
pArea->low_r_vnum = atoi(areadata[10]);
pArea->hi_r_vnum = atoi(areadata[11]);
pArea->low_o_vnum = atoi(areadata[12]);
pArea->hi_o_vnum = atoi(areadata[13]);
pArea->low_m_vnum = atoi(areadata[14]);
pArea->hi_m_vnum = atoi(areadata[15]);
pArea->low_soft_vnum = atoi(areadata[16]);
pArea->hi_soft_vnum = atoi(areadata[17]);
pArea->low_hard_vnum = atoi(areadata[18]);
pArea->hi_hard_vnum = atoi(areadata[19]);
pArea->author = areadata[20];
pArea->resetmsg = areadata[21];
pArea->max_players = atoi(areadata[22]);
pArea->mkills = atoi(areadata[23]);
pArea->mdeaths = atoi(areadata[24]);
pArea->pkills = atoi(areadata[25]);
pArea->pdeaths = atoi(areadata[26]);
pArea->gold_looted = atoi(areadata[27]);
pArea->illegal_pk = atoi(areadata[28]);
pArea->high_economy = atoi(areadata[29]);
pArea->low_economy = atoi(areadata[30]);
   LINK( pArea, first_area, last_area, next, prev );
   top_area++;
...
MYSQL_RES   *qresult;
MYSQL_ROW sysdata;
sprintf(selectquery, "SELECT * FROM sysdata");
qresult = query_db(selectquery);

   if (!sql_data_returned(qresult))
      return;
sysdata = mysql_fetch_row(qresult);
