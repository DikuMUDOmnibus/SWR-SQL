...
char * save_update( char * query, char * field, char * name )
{
sprintf(query, "%s %s ='%s',", query, field, name );
return query;
}
/*
...
/*
void load_obj( CHAR_DATA * ch, int id, short os_type )
{
   OBJ_DATA *obj;
  // char *word;
   int iNest;
...
MYSQL_ROW objdata;
   ...
   sprintf(selectquery, "SELECT * FROM objects where id='%d'", id);
qresult = query_db(selectquery);
while( (objdata = mysql_fetch_row(qresult)) )
   {
                      AFFECT_DATA *paf;
               int pafmod;
