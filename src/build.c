	   ...
	   if( !IS_IMMORTAL( victim ) ) 
         {
            ...
            save_clan( victim->pcdata->clan, CLANDATA_MEMBERS );
         }
...
/*	if ( get_trust(ch) >= sysdata.level_modify_proto )   */
   ...
   if( !tarea )
      for( tmp = first_build; tmp; tmp = tmp->next )
         ...
         {
/*		if ( get_trust(ch) >= sysdata.level_modify_proto  */
            if( get_trust( ch ) >= LEVEL_GREATER || is_name( tmp->name, ch->pcdata->bestowments ) )
            {
               tarea = tmp;
               break;
            }
...
void fold_area( AREA_DATA * tarea, char *filename, bool install )
{
   ...
   sprintf( buf, "Saving %s...", tarea->name );
...
/* Ensure not wiping out their area with save before load - Scryn 8/11 */
   ...
   sprintf( filename, "%s%s", BUILD_DIR, tarea->name );
...
void do_foldarea( CHAR_DATA * ch, char *argument )
{
   ...
   if( !str_cmp( arg, "all" ) )
   {
      for( tarea = first_area; tarea; tarea = tarea->next )
         fold_area( tarea, tarea->name, FALSE );
   ...
   for( tarea = first_area; tarea; tarea = tarea->next )
   {
      if( !str_cmp( tarea->name, arg ) )
      {
         ...
         if( !strcmp( argument, "remproto" ) )
            fold_area( tarea, tarea->name, TRUE );
         else
            fold_area( tarea, tarea->name, FALSE );
         send_to_char( "Done.\n\r", ch );
         return;
   ...
   for( tarea = first_build; tarea; tarea = tarea->next )
   {
     ...
     //    send_to_char( "Writing area.lst...\n\r", ch );
      //   save_areas(  );
         ...
         sprintf( buf, "%s%s.installed", BUILD_DIR, tarea->name );
         sprintf( arg, "%s%s", BUILD_DIR, tarea->name );
         rename( arg, buf );
         send_to_char( "Done.\n\r", ch );
         return;
      }
...
void do_astat( CHAR_DATA * ch, char *argument )
{
   ...
   ch_printf( ch, "Name: %s\n\rFilename: %-20s  Prototype: %s\n\r", tarea->name, tarea->name, proto ? "yes" : "no" );
   if( !proto )
   {
...
void do_aset( CHAR_DATA * ch, char *argument )
{
   ...
   if( !str_cmp( arg2, "planet" ) )
   {
      ...
      if( planet )
      {
         ...
         save_planet( planet, PLANETDATA_AREAS );
      }
      return;
   }
