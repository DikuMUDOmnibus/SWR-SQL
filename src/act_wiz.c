...
void do_advance( CHAR_DATA * ch, char *argument )
{
   ...
   {
      if( !str_prefix( arg3, ability_name[iAbility] ) && str_cmp(arg3, "all") )
      {
         ability = iAbility;
         break;
      }
...
if (!str_cmp( "all", arg3))
{
   ...
   for( iAbility = 0; iAbility < MAX_ABILITY; iAbility++ )
   {
                     victim->experience[iAbility] = exp_level(atoi(arg2));
      victim->skill_level[iAbility] = atoi(arg2);
gain_exp( victim, 0, iAbility );

}
      set_char_color( AT_WHITE + AT_BLINK, ch );
      ch_printf( ch, "All of your levels have been set at %d!\n\r", atoi(arg2) );
...
xSET_BIT(flags, SHIPDATA_VX);
xSET_BIT(flags, SHIPDATA_VY);
xSET_BIT(flags, SHIPDATA_VZ);
